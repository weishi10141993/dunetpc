#ifndef CHEATRECOPFOTRACKSELECTOR_H_SEEN
#define CHEATRECOPFOTRACKSELECTOR_H_SEEN
//STL
#include <iostream>
#include <limits>
//ROOT
...
namespace FDSelectionTools{
  class CheatRecoPfoTrackSelector : public RecoTrackSelector{
    public:
      explicit CheatRecoPfoTrackSelector(fhicl::ParameterSet const& ps);

    private:
      art::Ptr<recob::Track> SelectTrack(art::Event const & evt) override;

      std::string fNuGenModuleLabel;
      std::string fAllTrackModuleLabel;
      std::string fFittedTrackModuleLabel;
...
DEFINE_ART_CLASS_TOOL(FDSelectionTools::CheatRecoPfoTrackSelector)
#endif
